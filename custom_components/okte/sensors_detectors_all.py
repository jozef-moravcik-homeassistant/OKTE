...
*** Detector sensors for OKTE integration - Time Window Detectors ***
...
class OkteCheapestTimeWindowDetectorSensor(OkteBaseSensor):
    """Sensor - Detektor najlacnejšieho časového okna"""

    def __init__(self, hass: HomeAssistant):
        """Initialize the sensor."""
        super().__init__(hass, "cheapest_time_window_detector")
        self._attr_name = "Cheapest Time Window Detector"
    ...
    async def _handle_force_update(self, event):
        ...
        _LOGGER.debug("Force update received for cheapest time window detector")
        self._update_detector_state()
        self.async_write_ha_state()

    async def _hourly_update(self, now):
        """Update sensor state every hour at the top of the hour."""
        _LOGGER.debug("Hourly update triggered for cheapest time window detector")
        self._update_detector_state()
        self.async_write_ha_state()

    async def _handle_data_update(self, event):
        """Handle data update event."""
        _LOGGER.debug("Data update received for cheapest time window detector")
        self._update_detector_state()
    ...
    def _update_detector_state(self):
        """Update the detector state based on current time and cheapest window."""
        try:
            ...
            try:
                ...
                _LOGGER.debug(f"Cheapest window detector: {start_time_local} <= {now} < {end_time_local} = {self._state}")
...
class OkteMostExpensiveTimeWindowDetectorSensor(OkteBaseSensor):
    """Sensor - Detektor najdrahšieho časového okna"""

    def __init__(self, hass: HomeAssistant):
        """Initialize the sensor."""
        super().__init__(hass, "most_expensive_time_window_detector")
        self._attr_name = "Most Expensive Time Window Detector"
        self._attr_icon = "mdi:clock-check"
        self._attr_device_class = SensorDeviceClass.ENUM
        self._attr_options = ["true", "false"]
        self._state = False
    ...
    async def _handle_force_update(self, event):
        """Handle force update event for immediate detector refresh."""
        _LOGGER.debug("Force update received for most expensive time window detector")
        self._update_detector_state()
        self.async_write_ha_state()

    async def _hourly_update(self, now):
        """Update sensor state every hour at the top of the hour."""
        _LOGGER.debug("Hourly update triggered for most expensive time window detector")
        self._update_detector_state()
        self.async_write_ha_state()

    async def _handle_data_update(self, event):
        """Handle data update event."""
        _LOGGER.debug("Data update received for most expensive time window detector")
        self._update_detector_state()
        await super()._handle_data_update(event)
    ...
    def _update_detector_state(self):
        """Update the detector state based on current time and most expensive window."""
        try:
            ...
            try:
                ...
                _LOGGER.debug(f"Most expensive window detector: {start_time_local} <= {now} < {end_time_local} = {self._state}")
